...
from camera_utils import write_camera
...
def main(args):
    ...
    #SMPL-X
    t_frame = data_annots['SMPLx']['betas'][()].shape[0]
    frame_list = range(t_frame)

    smpl_dict = {}
    for key in ['betas', 'expression', 'fullpose', 'transl']:
        smpl_dict[key] = data_annots['SMPLx'][key][()][frame_list, ...]
    smpl_dict['scale'] = data_annots['SMPLx']['scale'][()]

    smpl_dir = os.path.join(input_path, 'smplx.npz')
    np.savez(smpl_dir, betas=smpl_dict['betas'], expression=smpl_dict['expression'], fullpose=smpl_dict['fullpose'], transl=smpl_dict['transl'], scale=smpl_dict['scale'])
    assert False
    # smpl_dir = os.path.join(input_path, 'smplx.csv')
    # with open(smpl_dir, 'w', newline='') as f:
    #     writer = csv.writer(f)
    ...
    write_camera(camera, input_path, args.output_basename)
