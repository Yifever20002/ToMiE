...
class ExtraJointsDeformer(nn.Module):
    def __init__(self,
                 mlp_width=128,
                 mlp_depth=4,
                 **_):
        super(ExtraJointsDeformer, self).__init__()
        # pose index embedding and time embedding
        embedding_size = p_dim
        print('ExtraJointsDeformer Embedding Input', embedding_size)
    ...
    def forward(self, p):
        ...
        input = p_feature
        d_joints = self.block_mlps(input).view(-1, 3)
